...
//  meter_set_string(&m1, 3, 2, "Hello");
//  meter_set_string(&m1, 10, 20, "Jerry");
  show_console();
  meter_update ();
...
main (void)
{
  ...
  gpio_set (GPIOC, GPIO12);     // PC12 = on

  // Need to set Port C-11 low to enable the USB port. Appears as ttyACM0
  gpio_set_mode (GPIOC, GPIO_MODE_OUTPUT_2_MHZ, GPIO_CNF_OUTPUT_PUSHPULL, USB_DISC);
  gpio_clear (GPIOC, USB_DISC); // USB DISC off

#if 0
  // Put SPI1 on Was PB5/PB4/PB3/PA15  // Was PB5/PB4/PB3/PA15
  ...
  gpio_clear (GPIOC, GPIO12);   // PC12 = off
