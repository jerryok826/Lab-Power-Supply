...
// indent -gnu -br -cli2 -lp -nut -l100 flash_rw_example.c
...
static void
usart_send_string (uint32_t usart, uint8_t * string, uint16_t str_size)
{
  uint16_t iter = 0;
  do {
...
static void
usart_get_string (uint32_t usart, uint8_t * out_string, uint16_t str_max_size)
{
  uint8_t sign = 0;
  uint16_t iter = 0;
...
static void
local_ltoa_hex (uint32_t value, uint8_t * out_string)
{
  uint8_t iter;
  ...
  *(out_string--) = 0;
  ...
  for (iter = 0; iter < 8; iter++) {
    *(out_string--) =
      (((value & 0xf) > 0x9) ? (0x40 + ((value & 0xf) - 0x9)) : (0x30 | (value & 0xf)));
    value >>= 4;
  }
}
